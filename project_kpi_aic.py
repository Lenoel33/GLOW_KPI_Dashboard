...
ALIASES: dict[str, list[str]] = {
    ...
    "outcome_approved": ["outcome approved", "approved outcome", "classification approved", "reviewed outcome", "approved", "approved one year outcome", "approved one-year outcome", "one year outcome approved", "one-year outcome approved"],
    "one_year_followup_due": ["one year follow up due", "one-year follow-up due", "one year followup due", "follow up due", "follow-up due", "due for one year follow up", "due for one-year follow-up"],
    "complete_assessment_set": ["complete assessment set", "complete assessment sets", "complete mmse gds sppb set", "complete mmse/gds/sppb set", "assessment set complete"],
...
def _project_mask(table: SourceTable, fields: dict[str, str], project: str) -> pd.Series:
    ...
    if project == "APRIL":
        ...
        lharmoni_specific = {
            ...
            "outcome_classification", "outcome_approved", "one_year_followup_due",
            "complete_assessment_set", "outcome_success", "lharmoni_track",
        }
...
def _lharmoni_record_level(
    ...
    for table in tables:
        ...
        if not mask.any():
            continue
        evidence["participants"] = evidence["participants"] or any(name in fields for name in ("enrolment_date", "participant_status", "person_name"))
        evidence["due"] = evidence["due"] or any(name in fields for name in ("enrolment_date", "one_year_followup_due"))
        evidence["outcomes"] = evidence["outcomes"] or any(name in fields for name in ("physical_outcome", "cognitive_outcome", "baseline_mmse", "followup_mmse", "baseline_sppb", "followup_sppb", "outcome_approved", "outcome_success"))
        centres = _centre_series(table, fields)
        entities = _entity_series(table.frame, fields, warnings, f"{table.source_file} / {table.source_sheet}")
        ...
        for idx in table.frame.index[mask]:
            ...
            # Participation is cumulative. A valid named person in a dedicated
            # L'Harmoni table is counted even when the enrolment date is missing.
            # Dated records are included when enrolled on or before the data cutoff.
            if status_ok and (pd.isna(enrolment) or enrolment <= data_cutoff):
                participants.setdefault(centre, set()).add(ent)

            # Prefer the source system's explicit one-year due flag. Fall back to
            # enrolment + 365 days only when that controlled field is unavailable.
            explicit_due = None
            if "one_year_followup_due" in fields:
                raw_due = table.frame.at[idx, fields["one_year_followup_due"]]
                if not _blank_value(raw_due):
                    explicit_due = _truthy(raw_due)
            if explicit_due is True or (explicit_due is None and pd.notna(enrolment) and enrolment + pd.Timedelta(days=365) <= data_cutoff):
            ...
            explicit_approved = "outcome_approved" in fields and _truthy(table.frame.at[idx, fields["outcome_approved"]])
            explicit_success = "outcome_success" in fields and _truthy(table.frame.at[idx, fields["outcome_success"]])
            explicit_complete = "complete_assessment_set" in fields and _truthy(table.frame.at[idx, fields["complete_assessment_set"]])
            ...
            has_classified_outcome = physical in {"Improved", "Maintained", "Declined"} or cognitive in {"Improved", "Maintained", "Declined"}
            # Explicit approved/complete source fields are sufficient evidence.
            # The dashboard checkbox only governs inferred timing-based outcomes.
            source_approved = explicit_approved or explicit_complete or (timing_rule_approved and rule_present and timing_valid)
            if source_approved and (has_classified_outcome or explicit_success):
...
def analyse_lharmoni(
    ...
    readiness = _readiness_table([
        ...
        ("60% improve or maintain physical/cognitive scores", totals.get("official_outcome_rate"), "Successful one-year physical/cognitive outcomes ÷ all tracked seniors due for one-year follow-up", True, "Uses explicit approved/complete outcome fields where available; otherwise the selected timing rule"),
    ...
    if not timing_rule_approved:
        warnings.append("The timing checkbox is off. Explicit approved/complete outcome fields are still counted; only inferred timing-based outcomes are excluded.")
    if not tables:
        errors.append("No readable structured project tables were found.")
    notes = {
...
def _diagnostic_trace(
    ...
    """Show a temporary end-to-end trace of the upload and KPI pipeline."""
    with st.expander(f"🔎 {project} diagnostic trace", expanded=True):
        st.caption("Temporary troubleshooting panel. It does not change any KPI calculations or store uploaded data.")
        ...
        for uploaded in files or []:
            ...
            uploaded_rows.append({
                "Uploaded file": getattr(uploaded, "name", "uploaded_file"),
                "Bytes": size,
            })
        st.markdown("**1. Files received by Streamlit**")
        if uploaded_rows:
            st.dataframe(pd.DataFrame(uploaded_rows), use_container_width=True, hide_index=True)
        else:
            st.error("No uploaded files reached the project page.")

        st.markdown("**2. File-reading result**")
        st.write(f"Readable tables detected: **{len(tables):,}**")
        if file_errors:
            for message in file_errors:
                st.warning(message)
        if not tables:
            st.error("The pipeline stopped because no readable table was created.")
            return

        detail_rows = []
        project_rows_total = 0
        named_rows_total = 0
        accepted_centre_rows_total = 0
        allowed = set(APRIL_CENTRES if project == "APRIL" else LHARMONI_CENTRES)
        for table in tables:
            fields = detect_fields(table.frame)
            project_mask = _project_mask(table, fields, project)
            centres = _centre_series(table, fields)
            warning_sink: list[str] = []
            entities = _entity_series(
                table.frame, fields, warning_sink,
                f"{table.source_file} / {table.source_sheet}",
            )
            project_count = int(project_mask.fillna(False).sum())
            named_count = int((project_mask.fillna(False) & entities.notna()).sum())
            accepted_count = int((project_mask.fillna(False) & centres.isin(allowed)).sum())
            project_rows_total += project_count
            named_rows_total += named_count
            accepted_centre_rows_total += accepted_count
            detail_rows.append({
                "Source file": table.source_file,
                "Sheet / table": table.source_sheet,
                "Rows": len(table.frame),
                "Detected fields": ", ".join(sorted(fields)) if fields else "None",
                f"{project} rows": project_count,
                "Rows with usable Name": named_count,
                "Rows in accepted centres": accepted_count,
                "Detected centre values": ", ".join(sorted({str(x) for x in centres.dropna().unique()})) or "None",
                "Name warnings": " | ".join(warning_sink) or "None",
            })

        st.markdown("**3. Tables, headers and project filtering**")
        st.dataframe(pd.DataFrame(detail_rows), use_container_width=True, hide_index=True)
        m1, m2, m3 = st.columns(3)
        m1.metric(f"Rows identified as {project}", project_rows_total)
        m2.metric("Rows with usable Name", named_rows_total)
        m3.metric("Rows in accepted centres", accepted_centre_rows_total)

        if project_rows_total == 0:
            st.error("Failure point: no rows were recognised as belonging to this project. Check the Project column, sheet name, or project-specific headers.")
        elif named_rows_total == 0:
            st.error("Failure point: project rows were found, but no usable Name values were detected.")
        elif accepted_centre_rows_total == 0:
            st.error("Failure point: project rows and names were found, but no rows matched the accepted centre rules.")
        else:
            st.success("The uploaded data passed file reading, project detection, Name detection and centre classification.")

        if result is not None:
            st.markdown("**4. KPI calculation output**")
            totals_df = pd.DataFrame([
                {"KPI key": key, "Calculated value": value, "Available": value is not None and not (isinstance(value, float) and np.isnan(value))}
                for key, value in result.totals.items()
            ])
            st.dataframe(totals_df, use_container_width=True, hide_index=True)
            unavailable = totals_df.loc[~totals_df["Available"], "KPI key"].tolist() if not totals_df.empty else []
            if unavailable:
                st.warning("Unavailable KPI keys: " + ", ".join(unavailable))
            else:
                st.success("All calculated KPI keys have values.")

            if result.warnings:
                st.markdown("**Calculation warnings**")
...
def render_april_page(template_path: Path | None = None, clear_all_callback=None) -> None:
    ...
    _diagnostic_trace("APRIL", files, tables, file_errors, result)
...
def render_lharmoni_page(template_path: Path | None = None, clear_all_callback=None) -> None:
    ...
    _diagnostic_trace("LHARMONI", files, tables, file_errors, cst_result)
