...
def _normalise_person_name(value: Any) -> str | pd._libs.missing.NAType:
    ...
    # Preserve Unicode letters and numbers (including Chinese names) while
    # removing punctuation and normalising repeated whitespace.
...
def _centre_series(table: SourceTable, fields: dict[str, str], project: str | None = None) -> pd.Series:
    hint = f"{table.source_file} {table.source_sheet}"
    if "centre" in fields:
        if project == "LHARMONI":
            # Confirmed L'Harmoni rule: use only the row-level Centres value.
            # Any value containing Bukit Batok/Nanyang maps to the corresponding
            # GLOW centre; filenames must never determine the centre.
            def map_lharmoni(value: Any) -> str | None:
                text = _key(value)
                has_bb = "bukit batok" in text
                has_ny = "nanyang" in text
                if has_bb and not has_ny:
                    return "GLOW Bukit Batok"
                if has_ny and not has_bb:
                    return "GLOW Nanyang"
                return None
            return table.frame[fields["centre"]].apply(map_lharmoni)
        return table.frame[fields["centre"]].apply(lambda v: canonical_centre(v, hint))
    if project == "LHARMONI":
        return pd.Series(None, index=table.frame.index, dtype="object")
...
def _latest_aggregate_values(
    ...
    for table in tables:
        ...
        centres = _centre_series(table, fields, project)
...
def _lharmoni_record_level(
    ...
    for table in tables:
        ...
        centres = _centre_series(table, fields, "LHARMONI")
        entities = _entity_series(table.frame, fields, warnings, f"{table.source_file} / {table.source_sheet}")
